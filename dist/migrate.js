...
async function migrate() {
    const schema = db_1.default.driver === "postgres" ? SCHEMA_PG : SCHEMA;
    await db_1.default.exec(schema);
    await ensureSmtpFromColumn();
    console.log(`✅  Migration complete (driver: ${db_1.default.driver})`);
    process.exit(0);
}
async function ensureSmtpFromColumn() {
    if (db_1.default.driver === "postgres") {
        await db_1.default.run("ALTER TABLE sites ADD COLUMN IF NOT EXISTS smtp_from TEXT");
        return;
    }
    const columns = await db_1.default.all("PRAGMA table_info(sites)");
    const hasColumn = columns.some((column) => column.name === "smtp_from");
    if (!hasColumn) {
        await db_1.default.run("ALTER TABLE sites ADD COLUMN smtp_from TEXT");
    }
}
migrate().catch((err) => {
